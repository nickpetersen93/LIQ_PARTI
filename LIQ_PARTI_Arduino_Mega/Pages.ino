...
void main_menu_page() {
  while (display_page == "main") {  //show main menu screen
    ...
    if (refresh_page) {  //do this only once every time main menu is opened
      ...
      drawBitmap(0, 40, PARTI_LOGOLIQ_PARTI_Logo, 240, 60, ILI9341_WHITE);
      ...
      if (!SD.begin(chipSelect)) {  //Info if SD card is instered, if not, --> display button to mount SD card to insert and retest
        ...
        DEBUG_System_PRINTLN("SD Card failed to initialize");
        eject_button = false;
    ...
    if (ts.touched()) {
      ...
      DEBUG_Screen_PRINT("(X = ");
      DEBUG_Screen_PRINT(p.x);
      DEBUG_Screen_PRINT(", Y = ");
      DEBUG_Screen_PRINT(p.y);
      DEBUG_Screen_PRINT(", Pressure = ");
      DEBUG_Screen_PRINT(p.z);
      DEBUG_Screen_PRINTLN(")");
  ...
  while (display_page == "recording") {
    ...
    while (LOG_COUNTER < (prev_min + LOG_INTERVAL)) {
      ...
      if (Wire.getWireTimeoutFlag()) {  //if the I2C timesout, rescue the sensors and rtc to continue
        ...
        DEBUG_System_PRINT("Timeouts, ");
        DEBUG_System_PRINTLN(timeouts);
        resetWire();
        if (!rtc.begin()) {
      ...
      read_tag_int();
      Record_Licks();  //record licks from MPR121 sensor #1
